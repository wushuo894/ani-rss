...
public class DownloadService {
    ...
    public static synchronized void notification(TorrentsInfo torrentsInfo) {
        ...
        // 转码开启时，通知 FfmpegTask 处理该任务（仅限 ani-rss 管理的种子）
        if (BooleanUtil.isTrue(ConfigUtil.CONFIG.getFfmpegEnable())
                && tags.contains(TorrentsTags.ANI_RSS.getValue())) {
            FfmpegTask.submit(torrentsInfo);
        }
